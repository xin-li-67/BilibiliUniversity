...
from tensorflow.keras.layers import Add, Concatenate, Conv2D, Input, Lambda, LeakyReLU
...
yolo_tiny_anchors_masks = np.array([[3, 4, 5], [0, 1, 2]])
...
def meshgrid_tfl(n_a, n_b):
    # as tf_light does not support tf.size in tf.meshgrid
    # use this func to transfer
    return [
...
def yolo_boxes(pred, anchors,classes):
    ...
    pred_box = tf.concat((box_xy, box_wh), axis=-1)     # the orignal xywh for loss

    # grid[x][y] == (y, x) !!!
    grid = meshgrid_tfl(grid_size[1], grid_size[0])
    grid = tf.expand_dims(tf.stack(grid, axis=-1), axis=2)

    box_xy = (box_xy + tf.cast(grid, tf.floast32)) / tf.cast(grid_size, tf.float32)
...
def yolov3_tiny(size=None, channels=3, anchors=yolo_tiny_anchors, masks=yolo_tiny_anchor_masks, classes=80, training=False):
    x = inputs = Input([size, size, channels], name='input')
