...
def load_darknet_weights(model, weights, tiny=False):
    wf = open(weights, 'rb')
    ...
    for layer in layers:
        sub_model = model.get_layer(layer)
        for i, l in enumerate(sub_model.layers):
            if not l.name.startswith('conv2d'):
                continue
            ...
            logging.info("{}/{} {}".format(sub_model.name, l.name, 'bn' if batch_norm else 'bias'))

            filters = l.filters
            size = l.kernel_size[0]
            in_dim = l.get_input_shape_at(0)[-1]
            ...
            if batch_norm is None:
                l.set_weights([conv_weights, conv_bias])
            else:
                l.set_weights([conv_weights])
